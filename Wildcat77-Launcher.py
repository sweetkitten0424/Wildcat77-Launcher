...
def download_fabric_installer(mc_version: str, logger, loader_version: str = "", installer_version: str = "") -> Path:
    ...
    try:
        ...
        if not installer_version or installer_version == "latest":
            latest_installer_data = installers[0]
            latest_installer_version = latest_installer_data["version"]
            installer_url = latest_installer_data["url"]
        else:
            # Find the specific installer version in the list
            found = False
            for installer_data in installers:
                if installer_data["version"] == installer_version:
                    latest_installer_version = installer_data["version"]
                    installer_url = installer_data["url"]
                    found = True
                    break
            if not found:
                raise RuntimeError(f"Fabric installer version {installer_version} not found")
        
        logger(f"Found Fabric Installer {latest_installer_version}")
        
        installer_name = f"fabric-installer-{latest_installer_version}.jar"
        installer_path = loaders_dir / installer_name
